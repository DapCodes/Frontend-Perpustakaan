...
class KategoriService {
  ...
  static Future<Kategori> listKategoris() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return Kategori.fromJson(jsonData);
  ...
  static Future<List<Datum>> fetchKategoris() async {
    try {
      final headers = await _getHeaders();
      final response = await http.get(
        Uri.parse("$baseUrl/kategoris"),
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['success'] == true && jsonData['data'] != null) {
          ...
          return dataList.map((json) => Datum.fromJson(json)).toList();
        } else {
          throw Exception(
  ...
  static Future<Datum?> fetchKategoriById(int id) async {
    try {
      final headers = await _getHeaders();
      final response = await http.get(
      ...
      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonData = json.decode(response.body);
        if (jsonData['success'] == true && jsonData['data'] != null) {
          return Datum.fromJson(jsonData['data']);
        } else {
  ...
  static Future<bool> createKategori(String namaKategori) async {
    try {
      final headers = await _getHeaders();
  ...
  static Future<bool> updateKategori(int id, String namaKategori) async {
    try {
      final headers = await _getHeaders();
